...
// very simple utility mesh representation for working with geometry on the CPU
// in C++
struct SimpleMesh {
  ...
  inline size_t TrianglesNum() const {
    return IndicesNum() / POINTS_IN_TRIANGLE;
  }
  inline void Resize(size_t a_vertNum, size_t a_indNum) {
    ...
    assert(a_indNum % 3 ==
           0); // NOTE: CURRENT IMPLEMENTATION ASSUMES ONLY TRIANGLE MESHES
  };
  ...
  inline size_t SizeInBytes() const {
    return vPos4f.size() * sizeof(float) * 4 +
           ...
           indices.size() * sizeof(int) + matIndices.size() * sizeof(int);
  }
  ...
  std::vector<unsigned int> indices;    // size = 3*TrianglesNum() for triangle
                                        // mesh, 4*TrianglesNum() for quad mesh
